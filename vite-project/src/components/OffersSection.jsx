...
export default function OffersSection({ lang = "en" }) {
  ...
  if (loading) {
    return (
      <section className="py-12 text-center text-gray-600">
        {lang === "en" ? "Loading offers..." : "சலுகைகள் ஏற்றப்படுகிறது..."}
      </section>
    );
  }
    ...
    return (
      <section className="py-12 text-center text-gray-600">
        {lang === "en"
          ...
          : "தற்போது சிறப்பு சலுகைகள் இல்லை."}
      </section>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 rounded-3xl
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-center mb-8 text-gray-800">
            {lang === "en" ? "🩺 Special Medical Offers" : "🩺 சிறப்பு மருத்துவ சலுகைகள்"}
          </h2>
                    ...
                    <div className="p-4 sm:p-5 space-y-3 sm:space-y-4">
                      {imageSrc ? (
                        ...
                        <div className="w-full h-36 sm:h-48 bg-gray-200
                          ...
                          {lang === "en" ? "No Image" : "படம் இல்லை"}
                        </div>
                      )}

                      <h3 className="text-lg sm:text-xl font-semibold truncate">
                        {lang === "en" ? offer.title?.en || offer.title : offer.title?.ta || offer.title}
                      </h3>

                      <p className="text-gray-600 text-sm sm:text-base truncate">
                        {lang === "en"
                          ? offer.description?.en || offer.description
                          : offer.description?.ta || offer.description}
                      </p>

                      <span className="inline-block px-3 py-1 text-xs sm:text-sm
